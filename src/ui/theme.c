                   ...
                   MetaTheme         *theme)
{
  switch (function)
    ...
    case META_BUTTON_FUNCTION_UNSHADE:
    case META_BUTTON_FUNCTION_UNABOVE:
      /* Fringe button that used to be supported by theme versions >1;
       * if we want to support them again, we need to return the
       * corresponding rects here
       */
      return NULL;
...
{
  meta_draw_op_list_draw_with_style (op_list, gtk_widget_get_style_context (widget),
                                     cr, info, rect);
}
