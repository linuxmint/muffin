...
meta_prefs_init (void)
{
  ...
  settings = g_settings_new (SCHEMA_MUFFIN_KEYBINDINGS);
  g_signal_connect (settings, "changed", G_CALLBACK (bindings_changed), NULL);
  g_hash_table_insert (settings_schemas,
                       g_strdup (SCHEMA_MUFFIN_KEYBINDINGS), settings);
...
init_bindings (void)
{
  ...
  keys = g_settings_list_keys (SETTINGS (SCHEMA_MUFFIN_KEYBINDINGS));
  for (i = 0; (guint)i < g_strv_length (keys); i++)
    {
      strokes = g_settings_get_strv (SETTINGS (SCHEMA_MUFFIN_KEYBINDINGS),
                                               keys[i]);
      update_key_binding (keys[i], strokes);
